...
class DililitvSpider(scrapy.Spider):
    name = 'hmtv'
    allowed_domains = ['www.hmtv.me']
    start_urls = [
        'https://www.hmtv.me/hanju'
    ]
    genres = {
        'dianying':1,
        'hanju':2,
    ...
    def __init__(self, pc=None, *args, **kwargs):
        ...
        if pc == None:
            ...
            self.start_urls = [ 'https://www.hmtv.me/new?t=' + tcode  ]
        else:
            pc = pc.strip()
            pc = int(pc) if pc.isdigit() else 0
            if pc > 1:
                for x in range(2, int(pc) + 1):
                    self.start_urls.append('https://www.hmtv.me/hanju/page/%s' % x)
    ...
    def parse_moviecontent(self, response, mItem):
        ...
        eplinks = response.xpath(u'//article[@class="article-content"]/div[@class="video_list_li"]/div[@class="vlink"]/a/@href').extract()
        ...
        for key, arg in diceplink:
            ...
            if arg.startswith('?Play='):
                ...
                vidurl = 'https://www.hmtv.me{0}'.format(arg)
            else:
                vidurl = 'https://www.hmtv.me/vplay/{0}.html'.format(arg)
